...
/**
 * 
 * @author Administrator to do : 根据 ID 花去trace 的 String 数组
 *
 */
public class subTrace extends JPanel {
	JFrame frame = new JFrame();
	String id;
	partTracePanel part1 = new partTracePanel(
			1,
			"2015/11/25 12:00 ：                                                             营业厅已揽件     上海");
	partTracePanel part2 = new partTracePanel(
			0,
			"2015/11/29 01:00 ：                                                            中转中心已接收  南京");
	String[] trace = {
			"2015/11/25 12:00 ：                                                             营业厅已揽件     上海",
			"2015/11/29 01:00 ：                                                            中转中心已接收  南京",
			"2015/11/29 01:00 ：                                                            快件已入库等待出库  南京",
			"2015/11/29 01:00 ：                                                            快件已出库  南京",
			"2015/11/29 01:00 ：                                                            中转中心已接收  南京",
			"2015/11/29 01:00 ：                                                            营业厅已接收  南京",
			"2015/11/29 01:00 ：                                                            阿呆派送中  南京" };
	Image title;
	Image background;
...
//		button.addActionListener(new Listener_Return());
	...
	public void getImage() {
		try {
			...
			background = ImageIO.read(new FileInputStream(
					"Trace/background.png"));
	...
	public class partTracePanel extends JPanel {
		...
		public void preState() {
			try {
				if (index == 1) {
					state = ImageIO.read(new FileInputStream(
							"Trace/accepetted.png"));
				} else {
					state = ImageIO
							.read(new FileInputStream("Trace/todeal.png"));
				}
			} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
